...
private:
	...
	Node* allocate(IntType x, Node *prev, Node *next)
	{
		if (freespace) {
			Node *node = freespace;
...
int main(int argc, char const *argv[])
{
	...
	//@ declare a decimal to store PI, each unit contains 9 digit
	Decimal<uint64_t, 9> pi(n, { 2 });
