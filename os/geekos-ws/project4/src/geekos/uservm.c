 ...
 */
bool Copy_User_Page(
    pde_t *page_dir, uint_t dest_user, char *src, uint_t byte_num) {
    uint_t       phy_start;
    uint_t       temp_len;
    int          page_nums;
    struct Page *page;
...
bool Alloc_User_Page(pde_t *pageDir, uint_t startAddress, uint_t sizeInMemory) {
    ...
    num_pages =
        Round_Up_To_Page(
            startAddress - Round_Down_To_Page(startAddress) + sizeInMemory)
        / PAGE_SIZE;
    ...
    for (i = 0; i < num_pages; i++) {
        if (!page_entry->present) {
            page_addr = Alloc_Pageable_Page(
                page_entry, Round_Down_To_Page(startAddress));
            if (page_addr == NULL) {
            ...
            *((uint_t *)page_entry)  = 0;
            ...
            KASSERT(page_addr != 0);
            if (i == 0) { first_page_addr = (uint_t)page_addr; }
        }
        page_entry++;
...
void Destroy_User_Context(struct User_Context *context) {
    ...
    if (context->pageDir != NULL) {
        ...
        pde_t *pdir;
        if (page_dir == NULL) { return true; }
        for (pdir = page_dir + NUM_PAGE_DIR_ENTRIES / 2;
             pdir < page_dir + NUM_PAGE_DIR_ENTRIES;
             pdir++) {
            pte_t *ptable;
            pte_t *ptable_first;
            if (!pdir->present) { continue; }
            ptable_first = (pte_t *)(pdir->pageTableBaseAddr << 12);
            for (ptable = ptable_first;
                 ptable < ptable_first + NUM_PAGE_TABLE_ENTRIES;
                 ptable++) {
                if (ptable->present) {
...
int Load_User_Program(
    ...
    for (i = 0; i < exeFormat->numSegments - 1; i++) {
        ...
        if (startAddress + sizeInMemory < USER_VM_LEN) {
            ...
            if (!Alloc_User_Page(
                    pageDirectory, startAddress + USER_VM_START, sizeInMemory)
                || !Copy_User_Page(
                    pageDirectory,
                    startAddress + USER_VM_START,
                    exeFileData + offsetInFile,
                    lengthInFile)) {
                return -1;
            }
        } else {
    ...
    if (!Alloc_User_Page(pageDirectory, arg_addr + USER_VM_START, arg_size)
        || !Copy_User_Page(
            pageDirectory, arg_addr + USER_VM_START, block_buffer, arg_size)) {
        return -1;
    }
    Free(block_buffer);
    ...
    if (!Alloc_User_Page(
            pageDirectory, stack_addr + USER_VM_START, DEFAULT_STACK_SIZE)) {
        return -1;
    }
    ...
    Init_LDT_Descriptor(
        uContext->ldtDescriptor, uContext->ldt, NUM_USER_LDT_ENTRIES);
    uContext->ldtSelector = Selector(
        USER_PRIVILEGE, true, Get_Descriptor_Index(uContext->ldtDescriptor));

    // 注意，在GeekOS的分页机制下，用户地址空间默认从线性地址2G开始
    Init_Code_Segment_Descriptor(
        &uContext->ldt[0],
        USER_VM_START,
        USER_VM_LEN / PAGE_SIZE,
        USER_PRIVILEGE);
